...
class Logger:
    def __init__(self, log_dir, wandb_object, checkpoint_freq=100, visualizer_params=None, zfill_num=8, log_file_name='log.txt'):
    ...
    def log_scores(self, loss_names):
        ...
        self.wandb.log({
            "perceptual loss": loss_mean[0],
            "equivariance value": loss_mean[1],
            "equivariance jacobian value": loss_mean[2]
        })
        if loss_mean[0] < self.lowest_loss:
            self.save_cpk(best=True)
    ...
    def save_cpk(self, emergent=False, best=False):
        cpk = {k: v.state_dict() for k, v in self.models.items()}
        cpk['epoch'] = self.epoch
        if best:
            cpk_path = os.path.join(self.cpk_dir, 'best-checkpoint.pth.tar')
        else:
            cpk_path = os.path.join(self.cpk_dir, '%s-checkpoint.pth.tar' % str(self.epoch).zfill(self.zfill_num)) 
            self.latest_checkpoint = cpk_path
